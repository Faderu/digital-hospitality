        ...
        <form action="{{ route('medicines.update', $medicine) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ $medicine->name }}" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="stock" id="stock" value="{{ $medicine->stock }}" min="0" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500">
                </div>

                <div class="col-span-2">
                    <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
                    <textarea name="description" id="description" rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500">{{ $medicine->description }}</textarea>
                </div>

                <div class="col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Gambar Obat</label>
                        ...
                        <div class="shrink-0">
                            <div class="relative w-32 h-32 bg-gray-50 rounded-lg border border-gray-200 flex items-center justify-center overflow-hidden">
                                <img id="img-preview" 
                                     ...
                                     alt="Preview" 
                                     class="{{ $medicine->image ? '' : 'hidden' }} w-full h-full object-cover">
                                
                                <div id="no-img-text" class="{{ $medicine->image ? 'hidden' : '' }} text-gray-400 text-xs text-center p-2">
                                    Belum ada foto
                                </div>
                            </div>
                            <p class="text-center text-xs text-gray-500 mt-1">Preview</p>
                        </div>

                        <div class="flex-1">
                            <label class="block text-sm text-gray-600 mb-1">Ganti Foto (Opsional)</label>
                            <input type="file" name="image" id="image" accept="image/*" onchange="previewEditImage(event)"
